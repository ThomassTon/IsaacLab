...
def approach_ee_handle(env: ManagerBasedRLEnv, threshold: float) -> torch.Tensor:
    ...
    ee_tcp_pos_2 = env.scene["ee_frame_2"].data.target_pos_w[..., 0, :]
    handle_pos = env.scene["cabinet_frame"].data.target_pos_w[..., 0, :]

    # Compute the distance of the end-effector to the handle
    distance = torch.norm(handle_pos - ee_tcp_pos, dim=-1, p=2)
    distance_2 = torch.norm(handle_pos - ee_tcp_pos_2, dim=-1, p=2)
    # Reward the robot for reaching the handle
    reward = 1.0 / (1.0 + distance**2)
    reward = torch.pow(reward, 2)
    
    reward_2 = 1.0 / (1.0 + distance_2**2)
    reward_2 = torch.pow(reward_2, 2)
    
    reward = torch.min(reward_2, reward)
...
def align_ee_handle(env: ManagerBasedRLEnv) -> torch.Tensor:
    ...
    ee_tcp_quat_2 = env.scene["ee_frame_2"].data.target_quat_w[..., 0, :]
    handle_quat = env.scene["cabinet_frame"].data.target_quat_w[..., 0, :]

    ee_tcp_rot_mat = matrix_from_quat(ee_tcp_quat)
    ee_tcp_rot_mat_2 = matrix_from_quat(ee_tcp_quat_2)
    ...
    ee_tcp_x_2, ee_tcp_z_2 = ee_tcp_rot_mat_2[..., 0], ee_tcp_rot_mat_2[..., 2]
    ...
    align_z_2 = torch.bmm(ee_tcp_z_2.unsqueeze(1), -handle_x.unsqueeze(-1)).squeeze(-1).squeeze(-1)
    align_x_2 = torch.bmm(ee_tcp_x_2.unsqueeze(1), -handle_y.unsqueeze(-1)).squeeze(-1).squeeze(-1)
    
    
    return torch.min(0.5 * (torch.sign(align_z) * align_z**2 + torch.sign(align_x) * align_x**2), 0.5 * (torch.sign(align_z_2) * align_z_2**2 + torch.sign(align_x_2) * align_x_2**2))
...
def align_grasp_around_handle(env: ManagerBasedRLEnv) -> torch.Tensor:
    ...
    ee_fingertips_w_2 = env.scene["ee_frame_2"].data.target_pos_w[..., 1:, :]

    lfinger_pos = ee_fingertips_w[..., 0, :]
    rfinger_pos = ee_fingertips_w[..., 1, :]

    lfinger_pos_2 = ee_fingertips_w_2[..., 0, :]
    rfinger_pos_2 = ee_fingertips_w_2[..., 1, :]
    
    # Check if hand is in a graspable pose
    is_graspable = (rfinger_pos[:, 2] < handle_pos[:, 2]) & (lfinger_pos[:, 2] > handle_pos[:, 2]) & (rfinger_pos_2[:, 2] < handle_pos[:, 2])& (lfinger_pos_2[:, 2] > handle_pos[:, 2])
...
def approach_gripper_handle(env: ManagerBasedRLEnv, offset: float = 0.04) -> torch.Tensor:
    ...
    lfinger_dist_2 = torch.abs(lfinger_pos_2[:, 2] - handle_pos[:, 2])
    rfinger_dist_2 = torch.abs(rfinger_pos_2[:, 2] - handle_pos[:, 2])

    # Check if hand is in a graspable pose
    is_graspable = (rfinger_pos[:, 2] < handle_pos[:, 2]) & (lfinger_pos[:, 2] > handle_pos[:, 2])
    is_graspable_2 = (rfinger_pos_2[:, 2] < handle_pos[:, 2]) & (lfinger_pos_2[:, 2] > handle_pos[:, 2])
    return torch.min(is_graspable * ((offset - lfinger_dist) + (offset - rfinger_dist)), is_graspable_2 * ((offset - lfinger_dist_2) + (offset - rfinger_dist_2)))


def grasp_handle(
    env: ManagerBasedRLEnv, threshold: float, open_joint_pos: float, asset_cfg: SceneEntityCfg, asset_cfg_2: SceneEntityCfg
) -> torch.Tensor:
    ...
    gripper_joint_pos_2 = env.scene[asset_cfg_2.name].data.joint_pos[:, asset_cfg_2.joint_ids]
    distance = torch.norm(handle_pos - ee_tcp_pos, dim=-1, p=2)
    distance_2 = torch.norm(handle_pos - ee_tcp_pos_2, dim=-1, p=2)
    is_close = distance <= threshold
    is_close_2 = distance_2 <= threshold
    return torch.min(is_close * torch.sum(open_joint_pos - gripper_joint_pos, dim=-1), is_close_2 * torch.sum(open_joint_pos - gripper_joint_pos_2, dim=-1))
...
def open_drawer_bonus(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    ...
    return (is_graspable) * drawer_pos    # (is_graspable + 1.0) * drawer_pos
